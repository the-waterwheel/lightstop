...
回答用户此前的问题：YUV 的 Y 分量当然能够反映画面相对亮度，但不能把编码后的 Y 直接当作线性光量。Camera2 对默认 `YUV_420_888` 定义为 JFIF/Rec.601 全范围，并说明转换后的 RGB 位于 sRGB 色彩空间；API 33 起还可以从帧 dataspace 读取显式范围和矩阵。厂商 FAST/HIGH_QUALITY ISP 仍可使用自己的 tone map 和非全局增强，因此绝对测光必须结合每帧结果元数据并保留来源校准。

旧算法把 `Y / 255` 当作 sRGB 后做固定 EOTF，跨设备可能受到以下因素影响：

- ISP tone curve 或局部色调增强；
- 局部 HDR/暗部提亮；
- AE、降噪和锐化策略；
- 显式 dataspace、厂商错误标注或非默认输出造成的 limited/full range 差异；
- 厂商在不同亮度区间使用不同非线性响应；
- 镜头切换时不同传感器/ISP pipeline 的响应差异。

正确定位是：

- Y 是非线性 RGB 域中的亮度编码；只读取 Y 会在高饱和红、蓝等区域产生颜色相关误差；
- 当前实现读取 Y/U/V 三平面，按 JFIF 或帧 dataspace 还原 RGB，优先反演 Camera2 返回的逐通道 tone curve，再计算 Rec.709 线性亮度中位数；
- 线性亮度 + 实际曝光时间 + 实际 ISO + 光圈，可以估算 EV；
- 每来源校准可以消除剩余常量偏移，但单点 offset 仍不能保证修正厂商未报告的局部/内容感知处理；
