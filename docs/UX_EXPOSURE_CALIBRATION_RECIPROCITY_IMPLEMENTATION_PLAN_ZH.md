...
状态：代码实施完成，待真机验收（2026-08-26）。历史 Zone 限制、摄影工具 UI、倒易率、EV100 方框、ISO 优先的曝光预览、隐私说明、tagged 中性 AE 恢复，以及三来源校准的存储、界面、顺序计划与安全会话隔离均已实现并通过单元测试和 debug 构建。跨厂商真机矩阵与最终 lint/USB 日志仍待验收；本文保留为后续工作的约束与验收清单。
制定日期：2026-08-26
...
- [x] 无 RAW 网格时 ZONE 明显置灰且完全不可交互。
- [x] 有 RAW 网格时历史触点重算不回归。
- [x] 参数、EV100、胶片和备注更大，长内容可滚动。

### 曝光预览与速度

- [x] 常规低光先提高 ISO，预览快门默认不慢于 1/30。
- [x] ISO 到顶后最多放慢到 1/15，再不足明确钳制。
- [x] 手动 frame duration 不继承上一帧慢值。
- [x] 开启曝光预览后的测光只等 2 个 tagged 中性稳定帧。
- [x] 正式测光仍严格同帧且不使用曝光预览帧冒充。

### 校准

- [x] RAW/YUV/ISP correction 独立存储和应用。
- [x] 高精度按顺序校准 RAW、YUV、ISP，不并发采集。
- [x] 稳定模式不为校准临时引入 YUV。
- [x] 旧 shared 数据安全迁移并在 UI 标明。
- [x] 曝光预览转换使用 ISP 显示域 correction。
- [x] 一次校准固定相机路由和修正存储身份；会话重开造成 active physical camera ID 短暂为空时不中止，但两个明确且不同的物理 ID 仍安全中止。
- [x] 覆盖逻辑主摄身份序列 `0@2 → 0 → 0@2` 的单元测试，确保 RAW 后可继续 YUV/ISP；另测两个明确物理 ID 不同必须中止。

### 主界面和摄影工具

- [x] 普通测光和 Zone 标点按钮左侧都有 EV100 方框。
- [x] Zone 显示最新当前标点，不显示加权平均。
- [x] 景深波轮只有固定顶部红指针，所有指定文字增大。
- [x] 胶片详情更大更深，宽容度轨道/标签/按钮更清楚。
- [x] 倒易率 1h 以上显示 `HH:MM:SS`，每列单位正确。
- [x] 动态滑块限制基于最终校正时间，精确数据例外只到数据截止。

### 工程质量

- [x] 没有新上千行大类。
- [x] 新策略为纯函数或单一状态机，可单测。
- [ ] 中英文、左右手、横竖屏、明暗主题和大字体通过（待真机逐页验收）。
- [x] README 与 `CAMERA_PIPELINE.md` 的校准说明同步。
- [ ] 自动测试、构建、lint（可执行时）和真机日志均有结果（待最终 lint 与 USB 验收）。
