...
- 枚举可用的逻辑相机和物理镜头；多摄设备默认使用更稳妥的自动逻辑相机，也可手动选择固定物理镜头；单色、红外等非普通成像传感器不会列入选择器。
- 预览优先选择不超过 1080 级别且接近传感器比例的尺寸；只从设备声明的 30 fps 及以下范围中选择，厂商拒绝时自动尝试 24 fps，最后交回系统默认。
...
- 设置提供三档：高精度（推荐）优先 RAW 并自动降级；兼容模式严格避免 RAW 与 YUV 同会话；快速模式只读取一次 ISP 处理后的预览。旧版本“兼容”设置会迁移为快速模式，保持升级前行为。

- RAW 测光按实际捕获 ISO 融合多帧：ISO ≤ 800 使用 3 帧，ISO > 800 使用 5 帧。
- RAW 保持 3/5 帧融合，但同一时刻只允许 1 张全尺寸 RAW 在途；当前图像关闭后才提交下一帧，以限制原生相机缓冲峰值。
...
- 兼容测光不做多帧降噪：优先读取 1 个 ISP 处理后的 YUV 有效帧；最多尝试 3 帧且总等待不超过 250 ms，随后立即读取 1 帧显示预览。
...
- 用户修正限制在 ±8 EV。
- 支持 RAW 的镜头会在一次校准中先完成 RAW、关闭图像资源，再立即完成兼容测光；不再使用固定阶段等待。没有 RAW 的镜头只校准兼容测光。
